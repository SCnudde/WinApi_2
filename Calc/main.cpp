...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		(GetModuleHandle(NULL), "ZERO.bmp", IMAGE_BITMAP, g_i_DOUBLE_BUTTON_SIZE, g_i_BUTTON_SIZE, LR_LOADFROMFILE);
	...
	case WM_COMMAND:
	{
		...
		static INT operation = 0;// потому что IDC интовый
		...
		if (LOWORD(wParam) >= IDC_BUTTON_PLUS && LOWORD(wParam) <= IDC_BUTTON_SLASH)
		{
			if (input)
			{
				if (a == DBL_MIN)a = atof(sz_buffer);	//atof= ANSI/ASCII to double	//https://legacy.cplusplus.com/reference/cstdlib/atof/?kw=atof
...
VOID SetSkinFromDLL(HWND hwnd, CONST CHAR skin[])
{
	...
	for (int i = IDC_BUTTON_0; i <= IDC_BUTTON_EQUAL; i++) 
	{
		...
		(hSkin,
			...
			LR_SHARED  //загрузка из разделяемой библиотеке
		);
